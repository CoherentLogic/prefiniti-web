...
<cfquery name="getCusts" datasource="sites">
	SELECT * FROM site_associations WHERE site_id=#URL.current_site_id# AND assoc_type=0
</cfquery>    

<cfquery name="getAddr" datasource="sites">
...
<div style="width:100%; background:url(/graphics/binary-bg.jpg); background-repeat:no-repeat; height:80px; border-bottom:2px solid ##EFEFEF; clear:right; ">
    ...
    <br />
    <br />
<form name="reqJob" action="job-submit.cfm" method="post" name="SurveyOrderForm" id="SurveyOrderForm">
	...
	<select name="custs" id="custs" onchange="SetValue('clientID', GetValue('custs'))">
...
Subdivision Plat</label>
		  ...
		  <td width="226"><input type="text" name="PlatCabinetBook" id="PlatCabinetBook" /></td>
	  </tr>
	  <tr>
	    <td><a href="javascript:loadHelp('Deed-Filing Date', 'DeedBookPage.cfm', 'DEED-PAGE.jpg');"><img src="/graphics/help.png" border="0" /></a></td>
		  ...
		  <td width="226"><input type="text" name="PageSlide" id="PageSlide" /></td>
	  </tr>
	  <tr>
	  ...
	  <tr>
		...
		<tr>
		  <td>&nbsp;</td>
			...
			<td colspan="3"><textarea name="specialinstructions" id="specialinstructions" rows="5" cols="50"></textarea></td>
		</tr>
		<tr>
		  <td>&nbsp;</td>
			<td>Your Job Number:</td>
			<td colspan="3"><input type="text" name="clientJobNumber" id="specialinstructions"/></td>
		</tr>
		<tr>
		  <td>&nbsp;</td>
		...
		<tr>
		  <td>&nbsp;</td>
        	<td>&nbsp;</td>
        	<td>
             	<label><input type="checkbox" id="request_photos" name="request_photos" />Request photos of property</label><br />
                <label><input type="checkbox" id="rfp" name="rfp" />Submit as Request for Proposal</label>
            </td>
        </tr>
		<tr>
		  <td>&nbsp;</td>
			...
			<td colspan="3" align="right">
                <input type="button" class="normalButton" name="Submit" value="Place Order or RFP" onclick="AjaxSubmitForm(AjaxGetElementReference('SurveyOrderForm'), '/workflow/components/survey_order_form_sub.cfm', 'tcTarget');" />            
        	</td>
		</tr>
	</table>
